...
//  Stage 2: processor_thread -> call policy API -> FIFO (取代原本的forward queue)
//  Stage 3: external forwarder reads FIFO and forwards to main broker
//
// Compile with:
                           ...
                           double *out_pval)
{
       ...
       json_object_object_get_ex(r,"p_value",&jp))
    {
        ...
        *out_pval  = json_object_get_double(jp);
        ...
        printf("[API] *** DECISION *** Action=%s, Trust=%.3f, P_value=%.6f\n", 
               action_str, *out_trust, *out_pval);
...
static void write_to_fifo(const char *ip, uint64_t count, double enqueue_ts) {
    ...
    if (len > 0 && len < sizeof(buffer)) {
        ...
        if (written == len) {
            ...
            printf("[FIFO] Partial write: %zd/%d bytes\n", written, len);
        }
    }
}
...
// ===== Stage 2: Processor Thread (receive_queue -> policy API -> fixed service time -> FIFO) =====
static void *processor_thread_fn(void *arg) {
    while(threads_running) {
        ...
        if (has_new_data) {
            ...
            double pre_fifo_ts = now_sec();
            // 如果是 forward，寫入 FIFO
            if (strcmp(action, "forward") == 0) {
                write_to_fifo(current_data.ip, current_data.packet_count, service_end_ts);
            }
            double pos_fifo_ts = now_sec();
            double fifo_write_ms = (pos_fifo_ts - pre_fifo_ts) * 1000.0;
            printf("[FIFO]: %.3fms\n", fifo_write_ms);
            // 定期清理
...
int mosquitto_plugin_init(mosquitto_plugin_id_t *identifier,
                          ...
                          int option_count)
{
    printf("[PLUGIN] Initializing three-stage FIFO plugin...\n");
    ...
    if (fifo_fd == -1) {
        ...
        printf("[PLUGIN] FIFO opened (blocking mode): %s\n", FIFO_PATH);
    }
    ...
    printf("[PLUGIN] initialized with three-stage pipeline (receive -> process -> FIFO)\n");
    fflush(stdout);
